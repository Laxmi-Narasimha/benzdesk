...
export default function LoginPage() {
    ...
    const { user, loading: authLoading, sendOtp, loginWithPassword, loginWithGoogle } = useAuth();
    ...
    const handleGoogleLogin = async () => {
        setIsLoading(true);
        // We pass redirectTo so it returns to the proper URL
        const { error } = await loginWithGoogle(redirectTo);
        if (error) {
            showError('Error', error || 'Failed to initialize Google Sign In');
            setIsLoading(false);
        }
    };
                    ...
                    {step === 'email' && (
                        <form onSubmit={handleEmailSubmit} className="p-6 space-y-5">
                            ...
                            <button
                                ...
                                disabled={isLoading}
                                className="w-full py-3.5 bg-white border border-gray-200 text-gray-700 font-semibold text-sm rounded-xl hover:bg-gray-50 transition-colors disabled:opacity-50 shadow-sm flex items-center justify-center gap-3"
                            >
