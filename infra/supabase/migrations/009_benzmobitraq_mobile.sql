-- ============================================================
-- BenzMobiTraq Mobile Backend Integration
-- Merged into BenzDesk with isolation precautions
-- ============================================================
...
-- ============================================================
-- MOBILE NOTIFICATIONS TABLE (Renamed from notifications)
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_mobile_notifications_role ON mobile_notifications(recipient_role);
CREATE INDEX IF NOT EXISTS idx_mobile_notifications_unread ON mobile_notifications(recipient_id, is_read) WHERE is_read = false;
CREATE INDEX IF NOT EXISTS idx_mobile_notifications_created ON mobile_notifications(created_at);
...
-- ============================================================
-- MOBILE APP SETTINGS TABLE (Renamed from app_settings)
-- ============================================================
...
INSERT INTO mobile_app_settings (key, value, description) VALUES
...
-- EMPLOYEES
CREATE POLICY "Users can view own profile" ON employees FOR SELECT USING (auth.uid() = id);
...
-- SHIFT SESSIONS
CREATE POLICY "Users can view own sessions" ON shift_sessions FOR SELECT USING (employee_id = auth.uid());
CREATE POLICY "Users can create own sessions" ON shift_sessions FOR INSERT WITH CHECK (employee_id = auth.uid());
CREATE POLICY "Users can update own sessions" ON shift_sessions FOR UPDATE USING (employee_id = auth.uid());
CREATE POLICY "Admins can view all sessions" ON shift_sessions FOR SELECT USING (EXISTS (SELECT 1 FROM employees WHERE id = auth.uid() AND role = 'admin'));

-- LOCATION POINTS
CREATE POLICY "Users can insert own locations" ON location_points FOR INSERT WITH CHECK (employee_id = auth.uid());
CREATE POLICY "Users can view own locations" ON location_points FOR SELECT USING (employee_id = auth.uid());
CREATE POLICY "Admins can view all locations" ON location_points FOR SELECT USING (EXISTS (SELECT 1 FROM employees WHERE id = auth.uid() AND role = 'admin'));

-- EMPLOYEE STATES
CREATE POLICY "Users can view own state" ON employee_states FOR SELECT USING (employee_id = auth.uid());
CREATE POLICY "Users can update own state" ON employee_states FOR UPDATE USING (employee_id = auth.uid());
CREATE POLICY "Users can insert own state" ON employee_states FOR INSERT WITH CHECK (employee_id = auth.uid());
CREATE POLICY "Admins can view all states" ON employee_states FOR SELECT USING (EXISTS (SELECT 1 FROM employees WHERE id = auth.uid() AND role = 'admin'));
...
CREATE POLICY "Users can view own notifications" ON mobile_notifications
    FOR SELECT USING (
        ...
        OR (
            ...
            AND EXISTS (SELECT 1 FROM employees WHERE id = auth.uid() AND role = 'admin')
        )
    );
...
-- EXPENSE CLAIMS
CREATE POLICY "Users can view own expenses" ON expense_claims FOR SELECT USING (employee_id = auth.uid());
CREATE POLICY "Users can create own expenses" ON expense_claims FOR INSERT WITH CHECK (employee_id = auth.uid());
CREATE POLICY "Users can update own expenses" ON expense_claims FOR UPDATE USING (employee_id = auth.uid() AND status = 'draft');
CREATE POLICY "Users can delete own draft expenses" ON expense_claims FOR DELETE USING (employee_id = auth.uid() AND status = 'draft');
CREATE POLICY "Admins can view all expenses" ON expense_claims FOR SELECT USING (EXISTS (SELECT 1 FROM employees WHERE id = auth.uid() AND role = 'admin'));
CREATE POLICY "Admins can update expenses" ON expense_claims FOR UPDATE USING (EXISTS (SELECT 1 FROM employees WHERE id = auth.uid() AND role = 'admin'));

-- EXPENSE ITEMS
CREATE POLICY "Users can view own expense items" ON expense_items
    FOR SELECT USING (EXISTS (SELECT 1 FROM expense_claims WHERE id = claim_id AND employee_id = auth.uid()));

CREATE POLICY "Users can insert own expense items" ON expense_items
    FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM expense_claims WHERE id = claim_id AND employee_id = auth.uid() AND status = 'draft'));

CREATE POLICY "Users can delete own expense items" ON expense_items
    FOR DELETE USING (EXISTS (SELECT 1 FROM expense_claims WHERE id = claim_id AND employee_id = auth.uid() AND status = 'draft'));

CREATE POLICY "Admins can view all expense items" ON expense_items
    FOR SELECT USING (EXISTS (SELECT 1 FROM employees WHERE id = auth.uid() AND role = 'admin'));

-- MOBILE APP SETTINGS
CREATE POLICY "Anyone can read settings" ON mobile_app_settings FOR SELECT USING (true);
...
-- ============================================================
-- STUCK DETECTION FUNCTION (Updated with new table names)
-- ============================================================
...
BEGIN
    ...
    FROM mobile_app_settings WHERE key = 'stuck_radius_meters';
    
    SELECT ((value::TEXT)::INTEGER * INTERVAL '1 minute') INTO stuck_duration 
    FROM mobile_app_settings WHERE key = 'stuck_duration_minutes';
...
-- Assuming pg_cron is available, schedule the stuck detection
...
CREATE TRIGGER update_sessions_updated_at BEFORE UPDATE ON shift_sessions FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_employee_states_updated_at BEFORE UPDATE ON employee_states FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_mobile_app_settings_updated_at BEFORE UPDATE ON mobile_app_settings FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
